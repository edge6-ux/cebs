...
// Domain-warped FBM noise — produces the swirling fluid/lava look
const frag = `
  ...
  void main() {
    ...
    // Slow smoke breath: sin oscillates 0→1→0 every ~45s, then we bias it
    // to spend more time near the dark end (pow > 1 pushes toward 0).
    float breath = sin(u_time * 0.14) * 0.5 + 0.5;
    breath = pow(breath, 0.4);

    // High exponent = dark regions expand and swallow the color.
    // At breath=0 (darkest) exponent=4.5, at breath=1 (brightest) exponent=1.6.
    float fPow = mix(4.5, 1.6, breath);
    f = pow(clamp(f * 1.3 - 0.1, 0.0, 1.0), fPow);

    // Black → deep purple → vivid violet → magenta (brand palette)
